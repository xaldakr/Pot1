...
namespace Pot1_API.Controllers
{
    ...
    public class TicketController : Controller
    {
        ...
        public IActionResult GetPorEstado([FromBody] JObject data)
        {
            ...
            try
            {
                // Extraer y validar las fechas
                inicio = data["inicio"]?.ToObject<DateTime>() ?? DateTime.Today;
                fin = data["fin"]?.ToObject<DateTime>() ?? DateTime.Today;

                // Convertir las fechas en el primer y último instante del respectivo día
                inicio = inicio.Date; // Primer instante del día
                fin = fin.Date.AddDays(1).AddTicks(-1); // Último instante del día
